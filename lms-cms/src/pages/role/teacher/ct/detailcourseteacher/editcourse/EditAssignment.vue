...
const fetchCourseAssignment = async () => {
    const id = route.params.id_course_assignment;
    try {
        const response = await axios.get(`/course-assignments/${id}`);
        form.value.title = response.data.title;
        form.value.description = response.data.description;
        courseSection.value = response.data.course_section.title;

        selectedAreas.value = response.data.skills?.map((skill) => ({
            label: skill.name,
            value: skill.id_skill,
        }));

        fileUploadData.value = response.data.resources;
    } catch (error) {
...
const submitAssignment = async () => {
    try {
        ...
        const response = await axios.post(`/course-assignments/${route.params.id_course_assignment}`, formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        });

        showToast('Updated assignment successfully!', 'success');
        setTimeout(() => {
                ...
                <div class="row">
                    <div class="col-md-12 mt-4 mt-md-0">
                        <div class="card rounded-2 p-4 border-0">
                            <h5 class="fw-light fs-16">Digitefa/Course Manajemen/Edit Assignment</h5>
                            <h4 class="fs-24">Course Manajemen</h4>
                            <div class="card p-3 bordersa mt-2">
                                <h5 class="fs-20">Edit Assignment</h5>
                                <hr />
                                <div class="mb-3 row">
                                ...
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-sm-9">
                                        <MultipleSelect v-if="skillsOptions.length && selectedAreas.length"
                                            :options="skillsOptions" v-model="selectedAreas"
                                ...
                                <div class="mb-0 row mt--18">
                                    ...
                                    <div class="col-sm-9">
                                        <FileUpload :filedata="fileUploadData" resourceType="assignments"
                                            v-model="uploadedFiles" />
                                    </div>
                                </div>
                                <div class="d-flex justify-content-end">
                                    <ButtonBiru class="ms-3 mb-4 h-40 px-4 rounded-3 fs-16" @click="submitAssignment">
                                        Submit
                                    </ButtonBiru>
                                </div>
                            </div>

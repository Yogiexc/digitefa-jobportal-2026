...
const fetchQuestions = async () => {
    ...
    try {
        ...
        questionData.value = questions.map((question) => {
            // Get reference_answer from options if it's an essay question
            let reference_answer = '';
            if (question.type === 'essay' && question.options && question.options.length > 0) {
                reference_answer = question.options[0].reference_answer || '';
            }

            return {
                ...question,
                // Add reference_answer directly to the question object

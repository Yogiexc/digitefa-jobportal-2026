...
const filteredData = computed(() => {
    ...
    } else if (selectedStatus.value === 'verified') {
        sortedData = sortedData.filter(student => !student.is_verified);
    }
...
const fetchUsersData = async () => {
    try {
        ...
        studentData.value = response.data.students.map(user => ({
            ...user,
            student: {
                ...user.student,
                image: user.student?.image
                    ? `${studentProfileURL}/${user.student.image}`
...
const goToEdit = (id) => {
    router.push(`/user-manajemen/student/detail/${id}`);
};
...
const submitForm = async () => {
    ...
    try {
        ...
        showToast('Add Student successfully!', 'success');

        await fetchUsersData();
        saveToLocalStorage(studentData.value);
        router.push('/user-manajemen/student');
    } catch (error) {
        showToast('Error Add student.', 'error');
        closeAddStudentModal();
    } finally {
...
const deleteStudent = async () => {
    ...
    try {
        if (currentStudent.value) {
            await axios.delete(`/users/${currentStudent.value.id_user}`);
            fetchUsersData();
            closeModal();
        ...
        <div class="navbg-sa">
            ...
            <div id="contentsa" class="dashboard-sa">
                        ...
                        <div class="col-md-12 mt-4 mt-md-0">
                            <div class="cbg-card rounded-2 p-4 border-0">
                                ...
                                <div class="d-flex justify-content-between mb-3 mt-4">
                                    ...
                                    <div>
                                        <ButtonBiru class="ms-3 h-40 px-3 rounded-3 fs-16" @click="showAddStudentModal">
                                            Add Student
                                        </ButtonBiru>
                                    </div>
                                </div>
                                        ...
                                        <table class="table custom-table rounded-4">
                                            <thead class="thead-custom">
                                                <tr class="ps-4">
                                                    ...
                                                    <th class="fs-16 fw-light w-400">Student</th>
                                                    <th class="fs-16 fw-light">Date of birth</th>
                                                    <th class="fs-16 fw-light">Address</th>
                                                    <th class="fs-16 fw-light">Phone Number</th>
                                                ...
                                                <tr v-for="(item, index) in paginatedData" :key="item.id">
                                                    ...
                                                    <td>
                                                        <div class="d-flex justify-content-start align-items-center gap-2">
                                                            <div class="">
                                                            <img v-if="item.student.image" :src="item.student.image"
                                                    ...
                                                    <td class="pt-20">{{ item.student.date_of_birth }}</td>
                                                    <td class="pt-20">{{ item.student.address }}</td>
                                                    <td class="pt-20">{{ item.student.phone }}</td>
                                                    <td class="ps-4 pt-20">
                ...
                <div v-if="isModalVisible" class="modal fade show d-block" role="dialog"
                    aria-labelledby="exampleModalLabel" aria-hidden="false" @click.self="closeAddStudentModal">
                        ...
                        <div class="modal-content">
                            <div class="modal-header mb--3">
                                ...
                                <button type="button" class="btn-close fs-12 c-close"
                                    @click="closeAddStudentModal"></button>
                            </div>
                            <hr class="mt-0">
                            <div class="px-3 mb-2">
                ...
                <!-- Delete Modal -->
                <div v-if="isDeleteModalVisible" class="modal-backdrop" @click="closeDeleteStudentModal">
                </div>
                <div v-if="isDeleteModalVisible" class="modal fade show d-block" role="dialog"
                    aria-labelledby="deleteModalLabel" aria-hidden="false" @click.self="closeDeleteStudentModal">
                    <div class="modal-dialog custom-modal modal-dialog-centered">
                        <div class="modal-content pt-3">
                            <div
                                ...
                                <h5 class="fs-16 fw-light opacity-50">
                                    Are you sure you want to delete this student? Once deleted, this
                                    data
                                    cannot be restored.
                                </h5>
                            </div>
                            <div class="d-flex justify-content-center mb-5">
                                <ButtonTransparanComponen class="my-0 h-40 w-30 me-5 rounded-3 c-border bg-white fs-16"
                                    @click="closeDeleteStudentModal">No, Cancel
                                </ButtonTransparanComponen>
                                <ButtonMerah class="ms-3 my-0 h-40 w-30 rounded-3 fs-16" @click="deleteStudent">Yes,
                                    Delete</ButtonMerah>
                            </div>
                        </div>
                    </div>

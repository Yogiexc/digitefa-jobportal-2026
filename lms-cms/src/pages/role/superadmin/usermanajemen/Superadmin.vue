...
const currentSuperadmin = ref(null);
const isDeleteModalVisible = ref(false);
const superadminToDelete = ref(null);
...
const filteredData = computed(() => {
    let sortedData = [...superadminData.value];
...
const loadFromLocalStorage = () => {
    ...
    if (storedData) {
        superadminData.value = JSON.parse(storedData);
    }
};
...
const fetchUsersData = async () => {
    try {
        ...
        superadminData.value = response.data.filter(user => user.role === 'admin');
        saveToLocalStorage(superadminData.value);
    } catch (error) {
...
const submitForm = async () => {
    ...
    try {
        ...
        showToast('Add Superadmin successfully!', 'success');

        await fetchUsersData();

        router.push('/user-manajemen/superadmin');
    } catch (error) {
        showToast('Error Add superadmin.', 'error');
        closeAddSuperadminModal();
    } finally {
...
const saveSuperadmin = async () => {
    ...
    try {
        if (currentSuperadmin.value) {
            const updatedData = {
                name: form.value.name || currentSuperadmin.value.name,
                email: form.value.email || currentSuperadmin.value.email,
            };

            if (Object.keys(updatedData).length > 0) {
                await axios.post(`/users/${currentSuperadmin.value.id_user}`, updatedData);
                fetchUsersData();
                closeEditSuperadminModal();
                showToast('Updated Akun successfully!', 'success');
            } else {
                showToast('No changes need to be saved.', 'error');
            }
        }
    } catch (error) {
        showToast('Error updating akun.', 'error');
    } finally {
        isLoading.value = false;
    }
};
...
const showEditSuperadminModal = (item) => {
    if (!item) {
        console.error("No superadmin selected for editing.");
        return;
    }
    currentSuperadmin.value = { ...item };
    form.value.name = currentSuperadmin.value.name || '';
    form.value.email = currentSuperadmin.value.email || '';
    hideDropdownMenu();
...
const closeEditSuperadminModal = () => {
...
const showDeleteSuperadminModal = () => {
    hideDropdownMenu();
...
const closeDeleteSuperadminModal = () => {
    isDeleteModalVisible.value = false;
    superadminToDelete.value = null;
...
const deleteSuperadmin = async () => {
    ...
    try {
        if (currentSuperadmin.value) {
            await axios.delete(`/users/${currentSuperadmin.value.id_user}`);
            fetchUsersData();
            closeModal();
            showToast('Superadmin deleted successfully!', 'success');
        }
    } catch (error) {
        showToast('Error deleting Superadmin.', 'error');
    } finally {
        isLoading.value = false;
    }
};
    ...
    <div>
        ...
        <div class="navbg-sa">
            ...
            <div id="contentsa" class="dashboard-sa">
                    ...
                    <div class="row">
                        <div class="col-md-12 mt-4 mt-md-0">
                            <div class="cbg-card rounded-2 p-4 border-0">
                                <h5 class="fw-light fs-16">Digitefa/Course Manajemen/Superadmin</h5>
                                <h4 class="fs-24">Superadmin</h4>
                                <div class="d-flex justify-content-between mb-3 mt-4">
                                    ...
                                    <div>
                                        <ButtonBiru class="ms-3 h-40 px-3 rounded-3 fs-16"
                                            @click="showAddSuperadminModal">
                                            Add Superadmin
                                        </ButtonBiru>
                                    </div>
                                </div>
                                        ...
                                        <table class="table custom-table rounded-4">
                                            <thead class="thead-custom">
                                                <tr class="ps-4">
                                                    ...
                                                    <th class="fs-16 fw-light w-400">Name Superadmin</th>
                                            ...
                                            <tbody class="table-custom">
                                                <tr v-for="(item, index) in paginatedData" :key="item.id">
                                                    <td class="ps-4">{{ (currentPage - 1) * itemsPerPage + index + 1 }}
                                                    </td>
                                                    <td>{{ item.name }}</td>
                                                    <td>{{ item.email }}</td>
                                                    <td class="ps-4">
                                                            ...
                                                            <ul v-if="dropdownVisible"
                                                                ...
                                                                <li>
                                                                    <a class="dropdown-item fw-normal fs-16 pointer"
                                                                        @click="showEditSuperadminModal(currentSuperadmin)">
                                                                ...
                                                                <li>
                                                                    <a class="dropdown-item fw-normal pointer"
                                                                        @click="showDeleteSuperadminModal">
                ...
                <!-- Add Modal -->
                <div v-if="isModalVisible" class="modal-backdrop" @click="closeAddSuperadminModal"></div>
                <div v-if="isModalVisible" class="modal fade show d-block" role="dialog"
                    aria-labelledby="exampleModalLabel" aria-hidden="true" @click.self="closeAddSuperadminModal">
                    <div class="modal-dialog custom-modal modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header mb--3">
                                <h5 class="fs-16 fw-medium" id="exampleModalLabel">
                                    <i class="bi bi-file-earmark-plus me-1"></i>Add Superadmin
                                </h5>
                                <button type="button" class="btn-close fs-12 c-close"
                                    @click="closeAddSuperadminModal"></button>
                            </div>
                            <hr class="mt-0">
                            <div class="px-3 mb-2">
                                <div class="d-flex justify-content-between mt-3">
                                    ...
                                    <input type="text" id="userName" class="form-control w-66 h-43"
                                        placeholder="Enter superadmin name" v-model="form.name" />
                                </div>
                                <div class="d-flex justify-content-between mt-3">
                            ...
                            <div class="d-flex justify-content-center mb-5">
                                <ButtonTransparanComponen
                                    ...
                                    @click="closeAddSuperadminModal">Cancel</ButtonTransparanComponen>
                ...
                <!-- Edit Modal -->
                <div v-if="isEditModalVisible" class="modal-backdrop" @click="closeEditSuperadminModal">
                </div>
                <div v-if="isEditModalVisible" class="modal fade show d-block" role="dialog"
                    aria-labelledby="exampleModalLabel" aria-hidden="false" @click.self="closeEditSuperadminModal">
                    <div class="modal-dialog custom-modal modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header mb--3">
                                <h5 class="fs-16 fw-medium" id="exampleModalLabel">
                                    <i class="bi bi-pencil-square me-1"></i>Edit Superadmin
                                </h5>
                                <button type="button" class="btn-close fs-12 c-close"
                                    @click="closeEditSuperadminModal"></button>
                            </div>
                            <hr class="mt-0">
                            <div class="px-3 mt-3 mb-2">
                                <div class="d-flex justify-content-between mt-3">
                                    <label for="superadminName" class="me-3 fs-16 mb-0 mt-2">Name</label>
                                    <input type="text" id="superadminName" class="form-control w-75 h-43"
                                        placeholder="Enter superadmin name" v-model="form.name" />
                                </div>
                                <div class="d-flex justify-content-between mt-3">
                            ...
                            <div class="d-flex justify-content-center mb-5">
                                <ButtonTransparanComponen
                                    class="mt-4 my-0 h-40 w-30 me-5 rounded-3 c-border bg-white fs-16"
                                    @click="closeEditSuperadminModal">Cancel</ButtonTransparanComponen>
                                <ButtonBiru class="ms-3 mt-4 my-0 h-40 w-30 rounded-3 fs-16" @click="saveSuperadmin">
                                    Save
                                </ButtonBiru>
                            </div>
                        </div>
                ...
                <!-- Delete Modal -->
                <div v-if="isDeleteModalVisible" class="modal-backdrop" @click="closeDeleteSuperadminModal">
                </div>
                <div v-if="isDeleteModalVisible" class="modal fade show d-block" role="dialog"
                    aria-labelledby="deleteModalLabel" aria-hidden="false" @click.self="closeDeleteSuperadminModal">
                    <div class="modal-dialog custom-modal modal-dialog-centered">
                        <div class="modal-content pt-3">
                            <div
                                ...
                                <h5 class="mb-4 mt-3 fs-16 fw-medium text-merah">Delete Superadmin</h5>
                                <h5 class="fs-16 fw-light opacity-50">
                                    Are you sure you want to delete this superadmin? Once deleted, this
                                    data
                                    cannot be restored.
                                </h5>
                            </div>
                            <div class="d-flex justify-content-center mb-5">
                                <ButtonTransparanComponen class="my-0 h-40 w-30 me-5 rounded-3 c-border bg-white fs-16"
                                    @click="closeDeleteSuperadminModal">No, Cancel
                                </ButtonTransparanComponen>
                                <ButtonMerah class="ms-3 my-0 h-40 w-30 rounded-3 fs-16" @click="deleteSuperadmin">Yes,
                                    Delete</ButtonMerah>
                            </div>
                        </div>
                    </div>

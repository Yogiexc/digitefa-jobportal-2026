...
for (let line of lines) {
    if (line.includes('def parse_cv(file: UploadFile = File(...)):')) {
        inParseCv = true;
    ...
    try:
        ...
        for line in sections["education"]:
            ...
            if not line: continue
            dates = extract_dates(line)
            if dates or current_edu is None or "univ" in line.toLowerCase() or "institut" in line.toLowerCase() or "school" in line.toLowerCase() or "sekolah" in line.toLowerCase():
        ...
        if !edu_list.length && sections["education"].length:
            edu_list.append({"university": "From CV", "degree": "", "major": "", "description": sections["education"].slice(0, 10).join(" "), "start_date": "", "end_date": ""})
    ...
    if (inParseCv) {
        if (line.includes('@app.get("/wordcloud")') || line.includes('def get_wordcloud():')) {
            inParseCv = false;
            newContent.push(line);
        }
    } else {
...
// Write the transformed back to file.
fs.writeFileSync(path, newContent.join('\n'));
console.log('main.py updated successfully using node.js logic');

...
const Show = ({ onBack, studentId }) => {
  ...
  const renderPersonalInformation = () => {
    return (
      <div className="mb-2">
        ...
        <Text className="font-medium text-xl">{jobSeekerData.full_name}</Text>
        <br />
        ...
        <Text className="text-xs">
          {new Date(jobSeekerData.date_of_birth).toLocaleDateString("en-GB", {
            day: "numeric",
            month: "long",
            year: "numeric",
          })}
  ...
  const renderPersonalSummary = (personal_summary) => {
    if (!personal_summary || personal_summary.length === 0) {
      return <Text>No personal summary available</Text>;
    }

    return (
      <div className="mb-2">
        <Text>{jobSeekerData.personal_summary}</Text>
      </div>
    );
  };
  ...
  const renderProjects = (projects) => {
    ...
    return projects.map((projects, index) => (
      <div key={index} className="mb-6 p-4 border rounded-xl">
        <Text className="font-medium text-xl">{projects.project_name}</Text>
        <br />
        <Text className="text-xs">
  ...
  return (
    <div className="bg-white shadow rounded-[20px] p-8">
      ...
      <Section
        ...
        content={renderPersonalSummary(jobSeekerData.personal_summary)}
      />
      <Section
        icon={EducationIcon}

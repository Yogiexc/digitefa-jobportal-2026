...
const ExportData = ({ open, setOpen }) => {
  ...
  const handleSave = async() => {
    form
      ...
      .then((values) => {
        Api.post(
          `/student/management/registered/export?format=${values.format}&start=${values.start}&end=${values.end}`,
          {
            responseType: "blob",
          }
        )
          .then((response) => {
            if (values.format === "xlsx") {
              saveAs(response, `registered-student.xlsx`);
            } else if (values.format === "csv") {
              saveAs(response, `registered-student.csv`);
            }
      ...
      >
        ...
        >
          ...
          <div className="mt-7" style={{ textAlign: "center" }}>
            ...
            >
              <span className="font-medium text-sm text-[#232323]">Cancel</span>
            </Button>
            <Button
              type="primary"
              onClick={handleSave}

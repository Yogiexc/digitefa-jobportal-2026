...
const ExportDataAppliedJob = ({ open, setOpen, jobSeekerId }) => {
  ...
  const handleSave = () => {
    form
      .validateFields()
      .then((values) => {
        Api.post(`/applied-jobs/${jobSeekerId}/export`, values, {
          responseType: "blob",
        })
        .then((response) => {
          if (values.format === "xlsx") {
            saveAs(response, `applied-jobs/${jobSeekerId}.xlsx`);
          } else if (values.format === "csv") {
            saveAs(response, `applied-jobs/${jobSeekerId}.csv`);
          }
...
export default ExportDataAppliedJob;

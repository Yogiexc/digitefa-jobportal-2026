...
const CompanyList = () => {
  ...
  const columns = [
    ...
    { title: "Company", dataIndex: "legal_name", key: "legal_name" },
    { title: "Registration Time", dataIndex: "created_at", key: "created_at", width: "18%",
      render: (text) => {
  ...
  const fetchData = (page, shouldFetchData) => {
    if (shouldFetchData) {
      setLoading(true);
      const url = `/companies/?page=${page}&pageSize=${pageSize}&sortBy=${sortBy}&sortOrder=${sortOrder}&search=${search}`;
      Api.get(url)
        .then((response) => {
  ...
  return (
    <Content className="p-6 -mt-3">
      ...
      <Table
        className="mt-4 overflow-x-auto max-w-full custom-table custom-table-wrapper"
        columns={columns}
        loading={loading}

...
const SkillsList = () => {
  ...
  const [selectedSkillsCategory, setSelectedSkillsCategory] = useState(null);
  ...
  const handleMenuClick = (record, action) => {
    if (action === "edit") {
      setSelectedSkillsCategory(record);
  ...
  const fetchData = (page, pageSize, sortBy, sortOrder, search) => {
    setLoading(true);
    const url = `/skills-category?page=${page}&pageSize=${pageSize}&sortBy=${sortBy}&sortOrder=${sortOrder}&search=${search}`;
    Api.get(url)
      .then((response) => {
  ...
  return (
    <Content className="p-6 -mt-3">
      ...
      <Table
        ...
        columns={columns}
        rowKey={(record) => record.skill_category_id}
        dataSource={data}
        loading={loading}
        pagination={false}
        footer={() => (
      ...
      <EditData
        ...
        skillsCategoryData={selectedSkillsCategory}
        fetchData={fetchData}
        setPage={setPage}
      />
      <DeleteData

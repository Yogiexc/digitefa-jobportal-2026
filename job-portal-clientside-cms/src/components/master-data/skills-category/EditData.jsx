...
const EditData = ({ open, setOpen, skillsCategoryData, fetchData }) => {
  ...
  const handleSave = () => {
    form
      .validateFields()
      .then((values) => {
        Api.put(`skills-category/${skillsCategoryData.skill_category_id}`, values)
          .then((res) => {
  ...
  useEffect(() => {
    if (skillsCategoryData) {
      form.setFieldsValue({
        category_name: skillsCategoryData.category_name,
      });
    }
  }, [skillsCategoryData]);

...
const StudentEmploymentRatio = () => {
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState([]);

  const fetchData = async () => {
    setLoading(true);
    await Api.get(`/dashboard/cms/university/student-employment-ratio`)
      .then((response) => {
      ...
      <div className="flex flex-col">
        ...
        <div className="flex justify-end mt-2">
          <Text className="text-[14px]" style={{ color: "#9A9A9A" }}>
            {data?.total_accepted} Out of {data?.total_talents} Talents have
            been accepted
          </Text>
        </div>
      </div>
...
export default StudentEmploymentRatio;

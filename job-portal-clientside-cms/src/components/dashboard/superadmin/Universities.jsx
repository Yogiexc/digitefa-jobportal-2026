...
const Universities = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const weekResponse = await Api.get(
          "/dashboard/cms/superadmin/total-universities?time=week"
        );
        const monthResponse = await Api.get(
         "/dashboard/cms/superadmin/total-universities?time=month"
        );
        const allTimeResponse = await Api.get(
          "/dashboard/cms/superadmin/total-universities?time=all-time"
        );
  ...
  function handleViewTalents() {
    navigateTo("/university-management");
  }
  return loading ? (
    ...
    <Card bordered={false} className="flex flex-col h-full">
      <div className="flex flex-col space-y-2">
        <div className="flex justify-between">          
            <Text className="text-[20px] font-semibold ">Universities</Text>
            ...
            alignValue === item.key ? (
              <div key={item.key}>
                <div className="flex flex-col">
                  ...
                  <Text className="text-[14px]" style={{ color: '#9A9A9A' }}>Total universities registered</Text>
                </div>
              </div>
            ) : null
          )}
      ...
      <div className="flex justify-start font-[#6B5312]">
          <Text
            ...
            style={{ color: '#6B5312' }}
          >
            University Management <ArrowRightIcon className="size-4 ml-2" />
          </Text>
        </div>
      </div>
    </Card>

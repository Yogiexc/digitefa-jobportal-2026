...
import { EyeIcon, EllipsisVerticalIcon, MagnifyingGlassIcon, CheckBadgeIcon } from "@heroicons/react/24/outline";
...
const UniversityList = () => {
  ...
  const handleMenuClick = (record, action) => {
    if (action === "show") {
      setSelectedUniversity(record.university_id);
  ...
  const fetchData = (page, shouldFetchData) => {
    if (shouldFetchData) {
      setLoading(true);
      const url = `/universities/management?page=${page}&pageSize=${pageSize}&sortBy=${sortBy}&sortOrder=${sortOrder}&search=${search}`;
      Api.get(url)
        .then((response) => {
      ...
      {openDetailView ? (
        ...
        <>
          ...
          <Table
            ...
            columns={columns}
            rowKey={(record) => record.university_id}
            dataSource={data}
            loading={loading}
            pagination={false}
            footer={() => (

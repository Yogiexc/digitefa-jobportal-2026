...
const EditData = ({
  ...
  const fetchData = (pageId) => {
    Api.get(`/content/event-news/id/${pageId}`)
      .then((response) => {
        ...
        form.setFieldsValue({
          ...
          event_date: data.category === "event" ? dayjs(data.event_date) : null,
        });

        setContent(data.content);
        setCategory(data.category);
        setShowDatePicker(data.category === "event");

        // Set the fileList if an image exists
        if (data.image_url) {
          setFileList([
            {
              uid: "-1", // A unique identifier for the file
              name: "Cover.png", // Name of the file
              status: "done", // Status indicating that the file is already uploaded
              url: data.image_url, // URL for the image file
            },
          ]);
  ...
  const handleSave = async () => {
    form.validateFields().then((values) => {
      ...
      Api.put(`/content/event-news/${pageId}`, formData, {
        headers: {

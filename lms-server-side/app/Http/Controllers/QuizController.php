...
class QuizController extends Controller
{
    ...
    {
        $quizzes = Quiz::with(['courseSection', 'resources', 'skills'])->get();
        return response()->json($quizzes);
    }

    public function show($id)
    {
        $quiz = Quiz::with(['courseSection', 'resources', 'skills', 'questions.resources', 'questions.options.resources'])->findOrFail($id);
    ...
    public function store(Request $request)
    {
        ...
        if (isset($validated['skills'])) {
            $quiz->skills()->sync($validated['skills']);
        }

        $uploadedFiles = [];
        if ($request->hasFile('files')) {
            $folderName = "Quiz Resources";
            $uploadedFiles = $this->uploadFilesToGoogleDrive($request->file('files'), $quiz->id_quiz, $folderName);
        }
        ...
        return response()->json([
            ...
            'quiz' => $quiz->load('skills', 'resources'),
    ...
    public function update(Request $request, $id)
    {
        ...
        $uploadedFiles = [];
        if ($request->hasFile('files')) {
            foreach ($quiz->resources as $existingResource) {
                try {
    ...
    public function indexByCourseSection($id_course_section)
    {
        $quizzes = Quiz::where('id_course_section', $id_course_section)->with('resources')->get();
        return response()->json($quizzes);
    }

    public function downloadAllResources($id)
    {
        $quiz = Quiz::with('resources')->findOrFail($id);
        $accessToken = $this->getGoogleAccessToken();

        return response()->stream(
            function () use ($quiz, $accessToken) {
                $zip = new \ZipStream\ZipStream(
                    outputName: $quiz->title . '_resources.zip',
            ...
            [
                ...
                'Content-Disposition' => 'attachment; filename="' . $quiz->title . '_resources.zip"',
            ]
        );
    }

    private function uploadFilesToGoogleDrive($files, $idQuiz, $folderName)
    {
        $uploadedFiles = [];
        $accessToken = $this->getGoogleAccessToken();
        ...
        foreach ($files as $file) {
            ...
            try {
                ...
                $uploadedFile = new QuizResource;
                $uploadedFile->name = $name;
                $uploadedFile->fileid = $fileId;
                $uploadedFile->id_quiz = $idQuiz;
                $uploadedFile->save();

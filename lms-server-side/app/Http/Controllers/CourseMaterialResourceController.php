...
class CourseMaterialResourceController extends Controller
{
    private function validateRequest(Request $request)
    {
        return $request->validate([
            'files.*' => 'required|file|mimes:jpeg,png,jpg,gif,pdf,doc,docx,txt,word,zip,rar|max:10240', // Max 10MB
            'id_course_material' => 'required|exists:course_materials,id_course_material',
        ]);
    }
            ...
            try {
                $folderCheckResponse = Http::withToken($accessToken)
                    ->get('https://www.googleapis.com/drive/v3/files', [
                        'q' => "name = 'Course Material Resources' and '{$mainFolderId}' in parents and mimeType = 'application/vnd.google-apps.folder' and trashed = false",
                        'fields' => 'files(id, name)'
                    ]);

                if (!$folderCheckResponse->successful()) {
                    return response('Failed to check for Course Material folder in Google Drive: ' . $folderCheckResponse->body(), 500);
                }

                $files = json_decode($folderCheckResponse->body())->files;
                $courseMaterialFolderId = $files[0]->id ?? null;

                if (!$courseMaterialFolderId) {
                    ...
                    if (!$folderCreateResponse->successful()) {
                        return response('Failed to create Course Material folder in Google Drive: ' . $folderCreateResponse->body(), 500);
                    }

                    $courseMaterialFolderId = json_decode($folderCreateResponse->body())->id;
                }

                $metadata = [
                    'name' => mb_convert_encoding($name, 'UTF-8', 'auto'),
                    'parents' => [$courseMaterialFolderId]
                ];
                ...
                if ($contentResponse->successful()) {
                    ...
                    $uploadedfile->id_course_material = $validatedData['id_course_material'];
                    $uploadedfile->save();
    ...
    {
        $file = CourseMaterialResource::findOrFail($id);
        $accessToken = $this->token();

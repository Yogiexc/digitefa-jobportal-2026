...
class AssignmentSubmissionResourceController extends Controller
{
    private function validateRequest(Request $request)
    {
        return $request->validate([
            'files.*' => 'required|file|mimes:jpeg,png,jpg,gif,pdf,doc,docx,txt,word,zip,rar|max:10240', // Max 10MB
            'id_assignment_submission' => 'required|exists:course_assignment_submissions,id_assignment_submission',
        ]);
    }
            ...
            try {
                $folderCheckResponse = Http::withToken($accessToken)
                    ->get('https://www.googleapis.com/drive/v3/files', [
                        'q' => "name = 'Course Assignment Submission' and '{$mainFolderId}' in parents and mimeType = 'application/vnd.google-apps.folder' and trashed = false",
                        'fields' => 'files(id, name)'
                    ]);

                if (!$folderCheckResponse->successful()) {
                    return response('Failed to check for Course Assignment Submission folder in Google Drive: ' . $folderCheckResponse->body(), 500);
                }

                $files = json_decode($folderCheckResponse->body())->files;
                $assignmentSubmissionResourcesFolderId = $files[0]->id ?? null;

                if (!$assignmentSubmissionResourcesFolderId) {
                    $folderCreateResponse = Http::withToken($accessToken)
                        ->withHeaders(['Content-Type' => 'application/json; charset=UTF-8'])
                        ->post('https://www.googleapis.com/drive/v3/files', [
                            'name' => 'Course Assignment Submission',
                            'mimeType' => 'application/vnd.google-apps.folder',
                            'parents' => [$mainFolderId],
                        ]);

                    if (!$folderCreateResponse->successful()) {
                        return response('Failed to create Course Assignment Submission folder in Google Drive: ' . $folderCreateResponse->body(), 500);
                    }

                    $assignmentSubmissionResourcesFolderId = json_decode($folderCreateResponse->body())->id;
                }

                $metadata = [
                    'name' => mb_convert_encoding($name, 'UTF-8', 'auto'),
                    'parents' => [$assignmentSubmissionResourcesFolderId]
                ];
                ...
                if ($contentResponse->successful()) {
                    ...
                    $uploadedfile->id_assignment_submission = $validatedData['id_assignment_submission'];
                    $uploadedfile->save();
    ...
    {
        $file = AssignmentSubmissionResource::findOrFail($id);
        $accessToken = $this->token();

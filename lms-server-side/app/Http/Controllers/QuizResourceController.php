...
class QuizResourceController extends Controller
{
            ...
            try {
                $folderCheckResponse = Http::withToken($accessToken)
                    ->get('https://www.googleapis.com/drive/v3/files', [
                        'q' => "name = 'Quiz Resources' and '{$mainFolderId}' in parents and mimeType = 'application/vnd.google-apps.folder' and trashed = false",
                        'fields' => 'files(id, name)'
                    ]);

                if (!$folderCheckResponse->successful()) {
                    return response('Failed to check for Quiz Resources folder in Google Drive: ' . $folderCheckResponse->body(), 500);
                }

                $files = json_decode($folderCheckResponse->body())->files;
                $quizResourcesFolderId = $files[0]->id ?? null;

                if (!$quizResourcesFolderId) {
                    ...
                    if (!$folderCreateResponse->successful()) {
                        return response('Failed to create Quiz Resources folder in Google Drive: ' . $folderCreateResponse->body(), 500);
                    }

                    $quizResourcesFolderId = json_decode($folderCreateResponse->body())->id;
                }

                $metadata = [
                    'name' => mb_convert_encoding($name, 'UTF-8', 'auto'),
                    'parents' => [$quizResourcesFolderId]
                ];
                ...
                if ($contentResponse->successful()) {
                    ...
                    $uploadedfile->id_quiz = $validatedData['id_quiz'];
                    $uploadedfile->save();
    ...
    {
        $file = QuizResource::findOrFail($id);
        $accessToken = $this->token();

...
class QuizQuestionResourceController extends Controller
{
    private function validateRequest(Request $request)
    {
        return $request->validate([
            'files.*' => 'required|file|mimes:jpeg,png,jpg,gif,pdf,doc,docx,txt,word,zip,rar|max:10240', // Max 10MB
            'id_quiz_question' => 'required|exists:quiz_questions,id_quiz_question',
        ]);
    }
            ...
            try {
                $folderCheckResponse = Http::withToken($accessToken)
                    ->get('https://www.googleapis.com/drive/v3/files', [
                        'q' => "name = 'Quiz Question Resources' and '{$mainFolderId}' in parents and mimeType = 'application/vnd.google-apps.folder' and trashed = false",
                        'fields' => 'files(id, name)'
                    ]);

                if (!$folderCheckResponse->successful()) {
                    return response('Failed to check for Quiz Question Resources folder in Google Drive: ' . $folderCheckResponse->body(), 500);
                }

                $files = json_decode($folderCheckResponse->body())->files;
                $quizQuestionResourcesFolderId = $files[0]->id ?? null;

                if (!$quizQuestionResourcesFolderId) {
                    $folderCreateResponse = Http::withToken($accessToken)
                        ->withHeaders(['Content-Type' => 'application/json; charset=UTF-8'])
                        ->post('https://www.googleapis.com/drive/v3/files', [
                            'name' => 'Quiz Question Resources',
                            'mimeType' => 'application/vnd.google-apps.folder',
                            'parents' => [$mainFolderId],
                        ]);

                    if (!$folderCreateResponse->successful()) {
                        return response('Failed to create Quiz Question Resources folder in Google Drive: ' . $folderCreateResponse->body(), 500);
                    }

                    $quizQuestionResourcesFolderId = json_decode($folderCreateResponse->body())->id;
                }

                $metadata = [
                    'name' => mb_convert_encoding($name, 'UTF-8', 'auto'),
                    'parents' => [$quizQuestionResourcesFolderId]
                ];
                ...
                if ($contentResponse->successful()) {
                    ...
                    $uploadedfile->id_quiz_question = $validatedData['id_quiz_question'];
                    $uploadedfile->save();
    ...
    {
        $file = QuizQuestionResource::findOrFail($id);
        $accessToken = $this->token();

...
class QuizOptionResourceController extends Controller
{
    private function validateRequest(Request $request)
    {
        return $request->validate([
            'files.*' => 'required|file|mimes:jpeg,png,jpg,gif,pdf,doc,docx,txt,word,zip,rar|max:10240', // Max 10MB
            'id_quiz_option' => 'required|exists:quiz_options,id_quiz_option',
        ]);
    }
            ...
            try {
                $folderCheckResponse = Http::withToken($accessToken)
                    ->get('https://www.googleapis.com/drive/v3/files', [
                        'q' => "name = 'Quiz Option Resources' and '{$mainFolderId}' in parents and mimeType = 'application/vnd.google-apps.folder' and trashed = false",
                        'fields' => 'files(id, name)'
                    ]);

                if (!$folderCheckResponse->successful()) {
                    return response('Failed to check for Quiz Option Resources folder in Google Drive: ' . $folderCheckResponse->body(), 500);
                }

                $files = json_decode($folderCheckResponse->body())->files;
                $quizOptionResourcesFolderId = $files[0]->id ?? null;

                if (!$quizOptionResourcesFolderId) {
                    $folderCreateResponse = Http::withToken($accessToken)
                        ->withHeaders(['Content-Type' => 'application/json; charset=UTF-8'])
                        ->post('https://www.googleapis.com/drive/v3/files', [
                            'name' => 'Quiz Option Resources',
                            'mimeType' => 'application/vnd.google-apps.folder',
                            'parents' => [$mainFolderId],
                        ]);

                    if (!$folderCreateResponse->successful()) {
                        return response('Failed to create Quiz Option Resources folder in Google Drive: ' . $folderCreateResponse->body(), 500);
                    }

                    $quizOptionResourcesFolderId = json_decode($folderCreateResponse->body())->id;
                }

                $metadata = [
                    'name' => mb_convert_encoding($name, 'UTF-8', 'auto'),
                    'parents' => [$quizOptionResourcesFolderId]
                ];
                ...
                if ($contentResponse->successful()) {
                    ...
                    $uploadedfile->id_quiz_option = $validatedData['id_quiz_option'];
                    $uploadedfile->save();
    ...
    {
        $file = QuizOptionResource::findOrFail($id);
        $accessToken = $this->token();

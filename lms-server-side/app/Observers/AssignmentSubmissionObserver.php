...
class AssignmentSubmissionObserver
{
    ...
    public function updated(AssignmentSubmission $submission)
    {
        if ($submission->isDirty('grade') && !is_null($submission->grade)) {
            $this->skillPointService->processAssignmentSkillPoints($submission);
            $this->checkAndUpdateEnrollmentProgress($submission->courseEnrollment);
        }
    }

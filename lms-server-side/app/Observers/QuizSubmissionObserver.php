...
class QuizSubmissionObserver
{
    ...
    public function updated(QuizSubmission $submission)
    {
        if (
            ($submission->isDirty(['status', 'grade']) &&
                $submission->status === 'completed' &&
                !is_null($submission->grade)) ||
            ($submission->isDirty('grade') &&
                $submission->status === 'completed' &&
                !is_null($submission->grade))
        ) {
            $this->skillPointService->processQuizSkillPoints($submission);
